        ...
        actions: [
          if (_mode != 'choose')
            TextButton.icon(
              ...
              label:
                  const Text('Reset', style: TextStyle(color: Colors.white70)),
            ),
          if (_isCalibrated)
            ElevatedButton.icon(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildModeCard(
                ...
                subtitle:
                    'Load a floor plan (PNG/JPG/PDF) and click to place anchors',
              ...
              _buildModeCard(
                ...
                subtitle:
                    'Enter room width and height (meters) to auto-generate a diagram',
                color: Colors.teal,
            ...
            if (_referencePoints.length < 2)
              ElevatedButton.icon(
                ...
                label: Text(
                  _isPlacingRefPoints
                      ? 'Click floor plan...'
                      : 'Mark Reference Distance',
                  style: const TextStyle(fontSize: 12),
                ),
                style: ElevatedButton.styleFrom(
                  backgroundColor: _isPlacingRefPoints
                      ? Colors.orange
                  ...
                  children: [
                    ...
                    Expanded(
                      child: SizedBox(
                        height: 32,
                        child: TextFormField(
                          initialValue: _referenceRealDistance > 0
                              ? _referenceRealDistance.toString()
                              : '',
                          decoration: const InputDecoration(
                            border: OutlineInputBorder(),
                            suffixText: 'm',
          ...
          children: [
            ...
            if (_placedAnchors.length >= 2)
              TextButton.icon(
                ...
                label:
                    const Text('Add Distance', style: TextStyle(fontSize: 12)),
              ),
          ],
        ),
        const SizedBox(height: 4),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          if (isRoomMode) ...[
            _buildStep(1, 'Click room diagram to place anchors (at least 1)',
                _placedAnchors.isNotEmpty),
            _buildStep(
                2, 'Coordinates auto-calculated from room size', _isCalibrated),
            _buildStep(3, 'Click "Apply to System" to finish', false),
          ] else if (isSingleAnchorFloorPlan) ...[
            _buildStep(
                1, 'Click canvas to place 1 anchor', _placedAnchors.isNotEmpty),
            _buildStep(
                2,
                'Click "Mark Reference Distance" to place 2 reference points',
                _referencePoints.length > 1),
            _buildStep(
                3,
                'Enter the real distance between the 2 reference points (meters)',
                _referenceRealDistance > 0),
            _buildStep(4, 'Click "Calculate Calibration"', _isCalibrated),
            _buildStep(5, 'Click "Apply to System" to finish', false),
          ] else ...[
            _buildStep(1, 'Click canvas to place anchors (at least 1)',
                _placedAnchors.isNotEmpty),
            if (_placedAnchors.length > 1) ...[
              _buildStep(2, 'Click 📏 to select anchor pairs',
                  _selectedAnchorIndex != null),
              _buildStep(3, 'Enter real distance between anchors (meters)',
                  _distancePairs.any((d) => d.distance > 0)),
            ],
            _buildStep(4, 'Click "Calculate Calibration"', _isCalibrated),
            _buildStep(5, 'Click "Apply to System" to finish', false),
  ...
  Future<void> _applyCalibration() async {
    ...
    if (_mode == 'floor_plan' &&
      ...
      if (box != null) {
        ...
        final originImageX = (_placedAnchors[0].pixelX - ox) / displayScale;
        final originImageY = (_placedAnchors[0].pixelY - oy) / displayScale;
        ...
        final offsetY = -(imgHeight - originImageY) * metersPerImagePixel;

        debugPrint('[Calibration] displayScale=$displayScale, ox=$ox, oy=$oy');
        debugPrint(
            '[Calibration] originImageX=$originImageX, originImageY=$originImageY');
        debugPrint(
            '[Calibration] metersPerImagePixel=$metersPerImagePixel, pixelsPerMeter=$pixelsPerMeter');
        ...
        debugPrint(
            '[Calibration] config after update: showFloorPlan=${uwb.config.showFloorPlan}, xScale=${uwb.config.xScale}, yScale=${uwb.config.yScale}, xOffset=${uwb.config.xOffset}, yOffset=${uwb.config.yOffset}');
      }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content:
              Text('✅ Calibration applied: ${_placedAnchors.length} anchor(s)'),
          backgroundColor: Colors.green,
        ),
      );
...
class _CalibrationPainter extends CustomPainter {
  ...
  void _drawAnchor(Canvas canvas, _CalibrationAnchor anchor, int index) {
    ...
    if (anchor.realX != null && anchor.realY != null) {
      ...
      coordTp.paint(
          canvas, Offset(pos.dx - coordTp.width / 2, pos.dy + 33 * s));
    }
  }
