    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildCheckboxRow(
              'Auto Get Anchor Coords', config.autoGetAnchorCoords, (v) {
          ...
          _buildCheckboxRow(
              'Show History Trajectory', config.showHistoryTrajectory, (v) {
            _updateConfig(config.copyWith(showHistoryTrajectory: v));
          }),
          _buildCheckboxRow('Trajectory/Navigation Mode', config.showTrajectory,
              (v) {
          ...
          _buildDropdownRow('Positioning Mode', config.positioningMode,
              ['2D Positioning', '3D Positioning'], (v) {
            _updateConfig(config.copyWith(positioningMode: v));
          }),
          _buildDropdownRow('Algorithm', config.algorithm,
              ['Kalman/Average', 'Least Squares', 'Trilateration'], (v) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: Text(widget.uwbService.isLoadingFloorPlan
                      ? 'Loading...'
                      : 'Open'),
                ),
              ),
                ...
                child: OutlinedButton.icon(
                  onPressed: config.floorPlanImagePath != null
                      ? () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text('Configuration auto-saved')),
                          );
                        }
                      : null,
          ...
          _buildNumberInputRowWithUnit('X Offset', _xOffsetController, 'm',
              (v) {
            _updateConfig(config.copyWith(xOffset: v));
          }),
          _buildNumberInputRowWithUnit('Y Offset', _yOffsetController, 'm',
              (v) {
            _updateConfig(config.copyWith(yOffset: v));
          }),
          const SizedBox(height: 16),
          _buildSectionTitle('Scale Settings'),
          _buildNumberInputRowWithUnit('X Scale', _xScaleController, 'px/m',
              (v) {
            _updateConfig(config.copyWith(xScale: v));
          }),
          _buildNumberInputRowWithUnit('Y Scale', _yScaleController, 'px/m',
              (v) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildNumberInputRowWithUnit('Height', _gridHeightController, 'm',
              (v) {
