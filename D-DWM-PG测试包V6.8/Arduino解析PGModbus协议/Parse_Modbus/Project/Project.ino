...
#define SERIAL_RX_MAX   128   //串口数据接收缓存长度
byte variable[SERIAL_RX_MAX]; //串口接收缓存区
  ...
  while(Serial.available() > 0)
  {
    ...
    if(index > 4)
    {
      ...
      else if(variable[0] == 0x01 && variable[1] == 0x03)  //modbusID号
      {
        ...
        if(ReceieveByte_length > index)  //还没接收完全
          break;
