...
namespace PGRtls.Log
{
    ...
    {      
            ...
            public void WriteLog(string logContent, LogType logType = LogType.Information, string fileName = null)
            {
                try
                {
                    string basePath = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);  //寻找程序运行文件夹
                   
                    if (!Directory.Exists(basePath + "\\Log"))   //建立Log文件夹
                    {
                        Directory.CreateDirectory(basePath + "\\Log");
                    }

                    string dataString = DateTime.Now.ToString("yyyy-MM-dd");  //建立时间文件夹
